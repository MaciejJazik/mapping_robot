 ...
 * YoloObjectDetector.cpp
 *
 *  Edited on: Dec 19, 2016
 *      Author: Jan Wiewiorowski
 *   Institute: UAM Poznan
 */
...
YoloObjectDetector::YoloObjectDetector(ros::NodeHandle nh)
    : nodeHandle_(nh), imageTransport_(nodeHandle_), numClasses_(0), classLabels_(0), rosBoxes_(0), rosBoxCounter_(0), imagergb_sub(imageTransport_,"/camera/rgb/image_raw",1), imagedepth_sub(imageTransport_,"/camera/depth/image_raw",1), sync_1(MySyncPolicy_1(5), imagergb_sub, imagedepth_sub)   {
...
void YoloObjectDetector::init() {
  ...
  std::string objectCoordinatesTopicName;
  int objectCoordinatesQueueSize;
  bool objectCoordinatesLatch;
  ...
  nodeHandle_.param("publishers/object_coordinates/topic", objectCoordinatesTopicName, std::string("object_coordinates"));
  nodeHandle_.param("publishers/object_coordinates/queue_size", objectCoordinatesQueueSize, 1);
  nodeHandle_.param("publishers/object_coordinates/latch", objectCoordinatesLatch, false);
  ...
  coordinatesPublisher_ =
      nodeHandle_.advertise<darknet_ros_msgs::ObjectCoordinates>(objectCoordinatesTopicName, objectCoordinatesQueueSize, objectCoordinatesLatch);
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          YoloObjectDetector::Coordinates(xmin, ymin, xmax, ymax);
...
void YoloObjectDetector::Coordinates(int xmin, int ymin, int xmax, int ymax)
{
  ...
  cv::GaussianBlur(imgBlurred, img, cv::Size(5, 5), 0);
  cv::cvtColor(imgBlurred, img, cv::COLOR_BGR2HSV);
